			...
			$.ajax({
				...
				$.each(aChangeFilesPaths, function (index, sFilePath) {
					...
					if (sChangesRelativePathIndex != -1 && sFilePath.endsWith(".change")) {
						if (sChangesRelativePathIndex != 0 && !sFilePath.startsWith(
								"fiori.manage.purchase.documents.purchasedocumentslrp.fiorielementsapp")) {
							return true;
						}
				...
				return Promise.all(aPromises).then(function (aChanges) {
					...
					var oLrepChange = {
						changes: oResult,
						componentClassName: "fiori.manage.purchase.documents.purchasedocumentslrp.fiorielementsapp"
					};
					resolve(oLrepChange);
				});
			});
